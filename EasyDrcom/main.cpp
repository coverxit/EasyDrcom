...
int read_config(std::string path)
{
	...
	conf.remote.dhcp = pt.get("Remote.DHCP", false);
	...
	conf.local.hostname = pt.get("Local.HostName", "EasyDrcom for MIPS");
		...
		try {
			do {
				...
				try {
					...
					if(conf.remote.dhcp) {
						//dhcp获取ip
						system("/sbin/dhcpc-renew");
						try {
							conf.local.ip = get_ip_address(conf.local.nic);
							SYS_LOG_INFO("Fetch NIC IP successfully." << std::endl);
							SYS_LOG_INFO("Local.IP = " << conf.local.ip << std::endl);
						} catch (std::exception& e) {
							SYS_LOG_ERR("Failed to fetch NIC IP - " << e.what() << std::endl);
							break;
						}
